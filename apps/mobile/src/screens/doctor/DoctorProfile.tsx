...
import { ChevronRight, Settings, Shield, LogOut, Bell } from 'lucide-react-native';
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
      <ScrollView className="flex-1">
        <View className="items-center py-8 bg-white border-b border-slate-200">
          <Avatar name={`Dr. ${user?.lastName}`} size={80} className="mb-4" />
          ...
          <View className="mt-4 bg-teal-50 px-4 py-1.5 rounded-full border border-teal-100">
            <Text className="text-teal-700 font-medium">Cardiology</Text>
          </View>
        </View>

        <View className="mt-6 px-4">
          <View className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm shadow-slate-100 elevation-1 mb-6">
            <View className="flex-row items-center justify-between p-4 border-b border-slate-100">
              <View className="flex-row items-center">
                <View className="w-10 h-10 bg-indigo-50 rounded-full items-center justify-center mr-4">
                  <Bell size={20} color={tokens.colors.primary.DEFAULT} />
                </View>
              ...
              <Switch 
                ...
                trackColor={{ false: '#cbd5e1', true: tokens.colors.primary.DEFAULT }}
              />
            </View>
            <ProfileOption icon={<Shield size={20} color={tokens.colors.primary.DEFAULT} />} title="Security & 2FA" />
            <ProfileOption icon={<Settings size={20} color={tokens.colors.primary.DEFAULT} />} title="App Settings" isLast />
          </View>
